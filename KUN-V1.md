...
**权威决策文档**：`decisions.md`（ADR-001 ~ ADR-024）——冲突时以该文件为准。

> ⚠️ **本文档 v2 修订自 2026-05-26 起生效，见 §0**。原章节中标有 🔁 的部分自该日起退役，行为依据迁移到 ADR-019 ~ ADR-024。

---

## §0 · v2 修订摘要（2026-05-26）

### 修订背景

2026-05-26 全盘审计发现 KUN-V1.md（v1）与实际实现存在 10 处内在矛盾，且原版几个核心叙事承诺（学习放每一面 / 守望大脑 / 两个大脑差异化 / 冷启动完整版）实际落地不到 30%。基于 5 轮深入讨论（架构 → 5 层重组 → 内部冲突自审 → RCDH 强制诊断 → Anti-drift 长任务防漂移），输出最终方案 v3，固化为 ADR-019 ~ ADR-024 六条决策。

本文档 v1 主体保留为**设计参考**；现行架构以下面 5 层 + 7 角色为准。

### 退役章节清单（行为依据迁到 ADR）

| 原章节 | 退役原因 | 替代依据 |
|------|------|------|
| §1.2 五条第一性原则（前 5 条保留，新增 6/7） | 补「Goal 优先于讨好」「诊断先于修复」 | ADR-020 / ADR-021 / ADR-022 |
| §1.1 第三条「帮用户赚钱 / 离钱近」 | 阶段化，Phase 2 启用 | PROGRESS.md L6 |
| §2.1 「不存在管理 agent」 | 改 agent 是显式实体按需扩展 | ADR-020 |
| §2.2 「两个大脑」二元 | 改主线 + 监督线双线 | ADR-020 |
| §2.3 黑板组件化讨论 | 降级为心智模型（不建 BlackboardView 组件） | ADR-020 |
| §6 整章「守望子系统是系统隐藏大脑」 | 改"监督线由多 agent 协作组成" | ADR-020 / ADR-023 |
| §8.7 surprise_score 单向偏置 | 改 task_anomaly_score 双向，喂给 Strategist | ADR-024 |
| §13 TASK.md L1/L2/L3 独立分级 | 与 complexity (simple/medium/complex) 合并 | ADR-020 |
| §15 M1-M5 里程碑命名 | 改 L0-L6 能力等级 | PROGRESS.md |
| §16.4 KnowledgePrecipitation 抽象 | 0 调用方，删；用 ADR-024 6 张表 + 10 步替代 | ADR-024 |
| 「冷启动上来就是完整版」承诺 | 改"骨架完整，跑 N 任务后展示能力" | PROGRESS.md L0/L1 |

### 新增机制（不在 v1 文档里，本次新建）

| 机制 | 决策出处 |
|------|---------|
| 五层架构（架构 / 运行 / 实例 / 升级 / 治理） | ADR-020 |
| 7 个 agent 角色（Director / Executor / Tester / Gate / Supervisor / Strategist / External Supervisor，按需扩展） | ADR-020 |
| 主线 / 监督线双线运行 | ADR-020 |
| RCDH 强制诊断层级（L0 设计 / L1 激活 / L2 模块 / L3 代码） | ADR-021 |
| Forward / Backward 双修复策略自动选择 | ADR-021 |
| Anti-drift 5 层工程化约束（GoalAnchor + Input Classifier + Heartbeat + Anti-sycophancy） | ADR-022 |
| External Supervisor 独立进程 + 本地模型 + 双模式（同步监管 + 任务尾复盘） | ADR-023 |
| RSI 闭环 10 步流程 + 6 张数据脊柱表 | ADR-024 |
| Auth 短/中/长三阶段 | ADR-019 |

### 七条核心原则（取代原 §1.2 五条）

1. **效果 > 速度 > 成本**（复杂任务）/ **效果 > 成本 > 速度**（简单任务）
2. **工程化约束 AI > 依赖模型能力**（不允许喂全 codebase 给 LLM 找问题）
3. **Goal 优先于讨好**（anti-sycophancy 工程化抑制讨好倾向）
4. **诊断先于修复**（RCDH 强制层级，重复 ≥ 3 次同症状强制升 L0/L1）
5. **闭环优于功能**（写了的功能必须有人读、有人反馈；建了没接的抽象立刻删或立刻接）
6. **透明可解释 + 可回滚**（任何决策有 evidence_ledger；任何启用有 rollback drill）
7. **人是终极仲裁**（RSI 允许全自动改 Executor / Tester；改 Supervisor / Strategist / Gate / Director 自己必须升 L4 人审）

退役原则：「不卖能力清单卖结果」「离钱近」「帮用户赚钱」「用户是决策合伙人」 — Phase 2 启用。

### 五层架构（取代原 §2 整章）

```
┌────────────────────────────────────────────────────────┐
│ L5 · 治理：RSI 闭环 (ADR-024) + RCDH (ADR-021)         │
│             + Anti-drift (ADR-022)                     │
├────────────────────────────────────────────────────────┤
│ L4 · 升级：4 级自治（角色 / 任务 / 监督线 / 人）       │
├────────────────────────────────────────────────────────┤
│ L3 · 实例：7 个 agent 角色（ADR-020）                  │
├────────────────────────────────────────────────────────┤
│ L2 · 运行：主线 + 监督线双线（ADR-020）                │
├────────────────────────────────────────────────────────┤
│ L1 · 架构：三元要素（Context / 接入层 / 工程化）       │
└────────────────────────────────────────────────────────┘
```

### 阅读建议

- **新读者**：先读本节 §0 + ADR-019 至 ADR-024 + PROGRESS.md（L0-L6） — 这是现行架构的最小集
- **历史参考**：v1 §1 至 §16 主体保留，但 §6 守望 / §2 架构 / §8.7 surprise 等章节以本节 §0 列出的 ADR 为准
- **冲突解决**：如遇本文档与 ADR-019 ~ ADR-024 矛盾，**以 ADR 为准**

---

## 一、产品定位与第一性原则

> 🔁 §1.1 第 3 条（帮用户赚钱）、§1.2 五条原则、§1.3 辅助原则的 UI 三铁律以外部分自 2026-05-26 起阶段化或被取代。原则更新版见 §0；现行原则以 ADR-019 ~ ADR-024 为准。
...
> 🔁 §2.1 三元要素保留；§2.1 末段「不存在管理 agent」自 2026-05-26 起退役（改 agent 是显式实体按需扩展，ADR-020）。
> 🔁 §2.2 「两个大脑」二元概念退役，改"主线 + 监督线双线"（ADR-020）。
> 🔁 §2.3 「黑板组件化」退役，降级为心智模型（ADR-020）。
...
> 🔁 整章自 2026-05-26 起重新定位。"守望子系统是系统隐藏大脑"表述退役 — 监督线由 Supervisor (傩 Pool) + Strategist (启 Pool) + External Supervisor 多 agent 协作组成（ADR-020）。
> 规则引擎 (§6.1.1) 保留为 Supervisor 的工具之一，但 4 类目录 (guard/validation/ci/anomaly) 中仅 guard + anomaly 保留 (ADR-018 §16.8 半合并状态)。
> 5 档干预 (§6.1) 保留为 GuardAction 枚举 (ADR-018 §16.6)，按 ≥ 3 调用方原则补齐。
> 监督线的核心运行机制以 **ADR-021 (RCDH) + ADR-022 (Anti-drift) + ADR-023 (External Supervisor)** 为准。
...
> 🔁 自 2026-05-26 起：
> - 改名 `task_anomaly_score`（"surprise" 是用户视角，"anomaly" 是系统视角）
> - `cost_dev` 改为 `abs(...)` 双向（省钱省时也是 insight，不只看超支）
> - 阈值不再触发用户 notification（user-facing 由 External Supervisor 主动 push）
> - 改为 **Strategist 的输入信号** — 异常 → 入 strategy_search_request 触发探索（ADR-024）
...
> 🔁 自 2026-05-26 起：
> - TASK.md L1/L2/L3 独立分级**与 complexity 合并**（simple/medium/complex 决定填充深度 + 加载深度）— Director 按 complexity 自适应实例化 TASK.md
> - 新增数据模型见 ADR-024 6 张数据脊柱表：runtime_capabilities / runtime_experiments / strategy_search_requests / diagnostic_records / goal_anchors / plan_reviews
> - evidence_ledger 加 3 字段（diagnostic_id / external_supervisor_debrief_id / diagnostic_level_reached）— ADR-021 + ADR-023
...
> 🔁 M1-M5 命名自 2026-05-26 起退役，改 **L0-L6 能力等级**（PROGRESS.md + ADR-020）。
> 历史 M1-M5 段保留供引用：M1 ≈ L0；M2 ≈ L2 部分；M3 ≈ L3/L4；M4 跨 Phase 按需做；M5 ≈ L5。
> 当前阶段：Phase 1（L0-L5 鲲对鲲，RSI 闭环）；后续 Phase 2（L6+ 商业化）。
...
> ❌ **本抽象自 2026-05-26 起删除**（0 调用方，纯壳）。"结果 → 知识"的流程改由 ADR-024 的 RSI 闭环 10 步 + 6 张数据脊柱表落地。capability_writeback / 方法论蒸馏 / 路由规律涌现各自独立写入，直到出现 ≥ 3 调用方再考虑合并。
