...
async def run_task(
    ...
    # 完整决策链。默认也经过 V5 多车道调度器，让普通任务、长周期任务、后台治理、
    # 外部动作和高风险任务不要互相抢资源。KUN_CHAT_SCHEDULER_ENABLED=0 可回退直跑。
    orchestrator = get_orchestrator(request.app)
    if os.getenv("KUN_CHAT_SCHEDULER_ENABLED", "1") == "1":
        result = await _run_chat_via_scheduler(
            request,
            user_message=user_message,
            tenant_id=tenant_id,
            user_id=user_id,
...
async def _run_chat_via_scheduler(
    request: Request,
    *,
    user_message: str,
    tenant_id: str,
    ...
    owner = Owner(tenant_id=tenant_id, user_id=user_id)
    task_ref = TaskRef(
        meta=TaskMeta(
            ...
            success_criteria_short=user_message[:180] or "chat task",
        ),
        spec=TaskSpec(
            goal_detail=user_message,
            success_metrics=["return a useful answer"],
            external_resources=[f"output_kind:{output_kind}"],
        ),
    )
    scheduler = get_multi_task_scheduler(request.app)
    scheduled_task_id = await scheduler.submit(task_ref)
    timeout_sec = int(os.getenv("KUN_CHAT_SCHEDULER_TIMEOUT_SEC", "300"))
    scheduled_result = await scheduler.wait_done(scheduled_task_id, timeout_sec=timeout_sec)
    if scheduled_result.status != "done":
        raise HTTPException(
            status_code=503,
            detail=f"scheduled chat task {scheduled_task_id} {scheduled_result.status}: {scheduled_result.error}",
        )
    output = scheduled_result.output
    if isinstance(output, TaskResult):
        return output
    if isinstance(output, dict):
        return TaskResult.model_validate(output)
    raise HTTPException(
        status_code=500,
        detail=f"scheduler runner returned unsupported output for {scheduled_task_id}",
    )
