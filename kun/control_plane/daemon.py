...
class ControlPlaneDaemon:
    ...
    def tick_once(
        ...
        *,
        ...
        observed_at = now or _now()
        selected_mission_ids = list(mission_ids) if mission_ids is not None else self._active_missions()
        ...
        *,
        ...
        try:
            while True:
                ...
                if (
                    active_config.stop_when_idle
                    and idle_ticks >= active_config.idle_ticks_to_stop
                ):
                    stopped_reason = "idle"
                    break
                if (
        ...
        self._save_service_state(
            ...
            DaemonServiceState(
                ...
                last_tick_recovered_work_item_ids=tick_reports[
                    -1
    ...
    def _apply_recovery(
        ...
        *,
        ...
        if plan.retry_work_item is not None:
            ...
            self._transition_to_queued(work_item.mission_id, subject_ref=plan.retry_work_item.work_item_id)
        if plan.recovery_work_item is not None:
            self.control_plane.work_items[
                plan.recovery_work_item.work_item_id
