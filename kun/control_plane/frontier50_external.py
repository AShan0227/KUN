...
FRONTIER50_DEFAULT_WORKDIR = Path(
    "/Users/slyvan/Documents/Codex/2026-05-08/5-7-1-2-abtext-10"
)
...
def load_frontier50_round_summary(
    ...
    *,
    ...
    return QiABRoundSummary(
        ...
        notes=[
            f"ignored {review_only_count} review-only learning ticket(s) on a passing round"
        ]
...
def _kun_gate_passed(*, report: dict[str, Any], health: dict[str, Any]) -> bool:
    ...
    return not any(
        isinstance(gap, dict)
        and (
            gap.get("severity") in {"error", "critical"}
            or float(gap.get("delta") or 0.0) >= 0.05
        )
