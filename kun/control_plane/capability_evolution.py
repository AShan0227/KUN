...
def build_capability_promotion(
    ...
    *,
    ...
    evidence_refs = _dedupe(
        [*candidate.evidence_refs, *[ref for item in selected for ref in item.evidence_refs]]
    )
    ...
    if block_reason is None:
        ...
        if missing_stages:
            hard_gate_failures.extend(
                f"{stage}_evaluation_missing_or_failed" for stage in missing_stages
            )
    ...
    if block_reason is not None:
        ...
        gate = _promotion_gate(
            ...
            hard_gate_failures=[
                *hard_gate_failures,
                *(
                    [finding.code for finding in nuo_report.findings]
                    if blocked_by_nuo and nuo_report
                    else []
                ),
            ],
            failure_category=_failure_category(
                blocked_by_nuo=blocked_by_nuo, hard_gate_failures=hard_gate_failures
            ),
        )
...
def build_capability_rollback(
    ...
    *,
    ...
    hard_gate_failures = failed_evaluation.hard_gate_failures or ["production_runtime_regression"]
...
def _promotion_gate(
    *,
    ...
    return GateEvaluation(
        ...
        task_plan_version=evaluations[-1].task_plan_version
        if evaluations
        else "capability-evolution",
