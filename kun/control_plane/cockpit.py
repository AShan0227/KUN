    ...
    *,
    ...
    return TaskCockpitView(
        ...
        technical_refs=_technical_refs(
            ...
            gate_ref=(user_summary.ready_work_item_ids and progress.latest_gate_ref)
            or progress.latest_gate_ref,
...
def _current_plan(
    ...
    *,
    ...
    plans = [plan for plan in control_plane.task_plans.values() if plan.mission_id == mission_id]
...
def _plan_summary(plan: TaskPlan | None, *, objective: str) -> TaskCockpitPlanSummary:
    if plan is None:
...
def _artifact_summary(
    ...
    delivery_manifests = [
        ...
        if manifest.kind == "delivery" or manifest.supports_delivery
    ]
    ...
    return TaskCockpitArtifactSummary(
        ...
        evidence_refs=_unique_ref_list(
            ref for manifest in manifests for ref in manifest.evidence_refs
        ),
...
def _recovery_actions(
    *,
    ...
    if summary.latest_failure_category in {
        "model_quality_failure",
        "evidence_failure",
        "plan_failure",
    }:
        actions.append("更新任务方案或能力候选后复测，质量不过门禁不能交付。")
    if gate is not None and gate.next_action in {
        "needs_repair",
        "needs_rollback",
        "needs_plan_change",
    }:
...
def _unique_ref_list(values: Iterable[str]) -> list[str]:
    seen: set[str] = set()
    result: list[str] = []
    for value in values:
