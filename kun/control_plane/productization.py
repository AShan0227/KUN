...
def audit_productization_code_boundary(
    ...
    *,
    ...
    normalized_paths = sorted(
        {
            _normalize_repo_path(path, root=root)
...
class ProductizationDogfoodRunner:
    ...
    runner_type = "agent"
    runner_identity = "kun-productization-dogfood-runner"
...
def _present_subsystems(
    *,
    ...
    if any(
        gate.created_by == "nuo" or gate.governance_signal.startswith("nuo_")
        for gate in gates
    ...
    if any(
        ...
        and "external_behavior_distillation" in artifact.supports
...
def _comparison_from_signal(signal: ExternalBehaviorSignal) -> ExternalBehaviorComparisonRecord:
    if signal.adoption_mode == "kun_native_runtime":
        ...
        decision = "merge"
        reason = "Behavior overlaps existing KUN protocols, so it should merge into the native contract."
        ...
        complexity_impact = "low"
        blockers = ["production profile must be consumed as governance guidance, not copied runtime code"]
    signal_ref = behavior_signal_ref(signal)
...
def _successful_dogfood_work_item_result(
    *,
    ...
    manifest = ArtifactManifest(
        ...
        created_by=work_item.owner,
        content_hash=_hash_payload(
            {"artifact": artifact.artifact_id, "subsystem": subsystem}
        ),
    )
    gate = GateEvaluation(
