...
class InMemoryControlPlane:
    ...
    def progress_report(self, mission_id: str) -> ControlPlaneProgressReport:
        ...
        return ControlPlaneProgressReport(
            ...
            next_ready_work_item_ids=[
                item.work_item_id
                for item in self._ready_work_items(mission_id)
            ],
    ...
    def _hydrate_from_store(self, store: ControlPlaneStore) -> None:
        ...
        self.contracts = {
            item.contract_id: item for item in store.list_execution_contracts()
        }
        ...
        self.artifacts = {
            item.artifact_id: item for item in store.list_artifact_records()
        }
        self.artifact_manifests = {
            item.manifest_id: item for item in store.list_artifact_manifests()
        }
        self.ledger_events = {
            item.event_id: item for item in store.list_ledger_events()
        }
        ...
        self._ledger_sequences = Counter(
            {
                mission_id: max(
                    (event.sequence for event in self.ledger_events.values() if event.mission_id == mission_id),
                    default=0,
                )
