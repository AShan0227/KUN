...
class LedgerEvent(BaseModel):
    ...
    def _strong_event_schema(self) -> LedgerEvent:
        ...
        if missing:
            ...
            raise ValueError(f"{self.event_type} ledger event missing payload fields: {missing_list}")
        return self
    ...
    def _enforce_north_star_and_traceability(self) -> GateEvaluation:
        ...
        if self.result_quality < result_threshold:
            ...
            if self.north_star_verdict == "pass":
                raise ValueError("north_star_verdict cannot pass when result_quality is below threshold")
        if self.next_action in {"ready_to_deliver", "accepted"} and not (
            ...
            raise ValueError("delivery/acceptance decisions require artifact and evidence/test/review refs")
        ...
        if self.learning_eligibility != "none" and not self.failure_category and self.stage != "learning":
    ...
    def _production_capability_requires_proof_and_rollback(self) -> CapabilityProfile:
        if self.promotion_stage in {"canary", "production"}:
            if not (self.evidence_refs and self.holdout_refs and self.regression_refs):
                raise ValueError("canary/production capabilities require evidence, holdout, regression refs")
