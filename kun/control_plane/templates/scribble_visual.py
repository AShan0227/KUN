...
def visual_product_ready_files() -> dict[str, str]:
    ...
    return {
        ...
        "public/assets/world-rainbow-island.svg": _world_svg(
            "#91e4ff", "#d8f5a2", "#ffbf69", "rainbow"
        ),
        ...
        "public/assets/object-vehicle.svg": _object_svg("#56c2ff", "vehicle"),
        "public/assets/object-music.svg": _object_svg("#ffd166", "music"),
...
export default function App() {
  ...
  return (
    <main className="tabletWorkbench visual-polish-ready" data-playtest="scribble-parity">
      ...
      {mode === "play" ? (
        <section className="playGrid visualGameShell immersiveGameShell">
          <aside className="panel explorationMap floatingQuestRail">
            <h2>开放探索地图</h2>
          ...
          <section className="stagePanel visualStage immersiveStage" style={{ "--sky": activeWorld.palette.sky, "--ground": activeWorld.palette.ground, "--accent": activeWorld.palette.accent } as CSSProperties}>
            ...
            <div className="stageScene illustratedScene visualFocusLayer">
              ...
              <div className="starterObjectPreview" aria-label="可尝试的造物图标">
                <img src="/assets/object-bridge.svg" alt="桥" />
                <img src="/assets/object-light.svg" alt="灯" />
                <img src="/assets/object-magic.svg" alt="魔法物件" />
              </div>
          ...
          <aside className="panel floatingCreatorTray">
...
export function objectAsset(object: GeneratedObject): string {
  ...
  if (object.kind.includes("car") || object.kind.includes("train") || object.kind.includes("vehicle") || object.properties.includes("fast")) return "/assets/object-vehicle.svg";
  if (object.kind.includes("music") || object.kind.includes("song") || object.properties.includes("singing")) return "/assets/object-music.svg";
...
def _styles_css() -> str:
    return """*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#e9f5f9;color:#182333}.tabletWorkbench{min-height:100vh}.topBar{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:14px 20px;border-bottom:2px solid #203142;background:#fffaf0;position:sticky;top:0;z-index:5;box-shadow:0 8px 24px rgba(18,35,51,.12)}.brandLockup{display:flex;align-items:center;gap:12px}.brandMark{width:44px;height:44px;border-radius:13px;display:grid;place-items:center;background:#ff8c6a;color:white;font-weight:1000;font-size:24px;border:3px solid #203142;box-shadow:4px 4px 0 #203142}.topBar strong{display:block;font-size:22px;letter-spacing:0}.topBar span{color:#607080}.topBar button,.panel button,.ideaForm button,.starterIdeas button,.inventoryShelf button{border:2px solid #203142;background:white;border-radius:8px;padding:10px 12px;font-weight:900;color:#233247;box-shadow:3px 3px 0 #203142;cursor:pointer}.topBar button:hover,.selected{border-color:#1887ff!important;background:#eaf4ff!important;transform:translateY(-1px)}.playGrid,.immersiveGameShell{display:grid;grid-template-columns:minmax(0,1fr);gap:12px;padding:14px;position:relative}.immersiveGameShell{min-height:calc(100vh - 76px)}.floatingQuestRail,.floatingCreatorTray{position:absolute;z-index:4;width:min(260px,20vw);max-height:calc(100vh - 112px);overflow:auto;background:rgba(255,253,247,.9);backdrop-filter:blur(10px)}.floatingQuestRail{left:18px;top:18px}.floatingCreatorTray{right:18px;top:18px}.panel,.stagePanel,.parentBook,.testPanel{background:#fffdf7;border:2px solid #203142;border-radius:8px;padding:14px;box-shadow:5px 5px 0 rgba(32,49,66,.18)}.stagePanel{margin-left:calc(min(260px,20vw) + 20px);margin-right:calc(min(260px,20vw) + 20px);min-width:0}.explorationMap h2,.floatingCreatorTray h2{margin:6px 0 10px}.mapTile{display:grid!important;grid-template-columns:52px 1fr auto;align-items:center;gap:9px;width:100%;margin:0 0 10px;text-align:left}.mapTile img{width:52px;height:42px;object-fit:cover;border-radius:7px;border:1px solid #d3e2ef}.mapTile small{font-weight:1000;color:#1887ff}.worldHeader{display:grid;grid-template-columns:74px minmax(0,1fr) auto;gap:10px;align-items:center;margin-bottom:8px}.worldHeader h1{margin:2px 0;font-size:28px}.worldHeader p{margin:4px 0 0}.chapterPill{display:inline-flex;border:2px solid #203142;border-radius:999px;background:#fff;padding:4px 8px;font-weight:900;color:#405166}.companionPortrait{display:grid;justify-items:center;gap:3px;font-weight:1000}.companionPortrait img{width:64px;height:64px;filter:drop-shadow(3px 5px 0 rgba(32,49,66,.24))}.shardBadge{border:2px solid #203142;background:#fff6bf;border-radius:8px;padding:8px;box-shadow:3px 3px 0 #203142}.stageScene{height:clamp(280px,calc(100vh - 430px),430px);border-radius:8px;background:linear-gradient(var(--sky),#fff 52%,var(--ground));position:relative;overflow:hidden;border:2px solid #203142}.worldBackdrop{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.95}.stageCompanionAvatar{position:absolute;left:50%;bottom:96px;z-index:3;width:78px;height:78px;transform:translateX(-50%);filter:drop-shadow(5px 8px 0 rgba(32,49,66,.24));animation:bouncy 2.8s ease-in-out infinite}.starterObjectPreview{position:absolute;left:50%;bottom:48px;z-index:3;display:flex;gap:8px;transform:translateX(-50%);padding:6px 8px;border:2px solid #203142;border-radius:999px;background:rgba(255,255,255,.86);box-shadow:4px 4px 0 rgba(32,49,66,.18)}.starterObjectPreview img{width:36px;height:36px;object-fit:contain}.sceneLayer{position:absolute;inset:78px clamp(20px,17%,140px) 118px clamp(20px,17%,140px);z-index:2}.objectCard{position:absolute;display:grid;justify-items:center;gap:3px;min-width:104px;min-height:102px;border:2px solid #203142;border-radius:14px;background:rgba(255,255,255,.84);box-shadow:5px 7px 0 rgba(32,49,66,.18);padding:7px}.objectCard img{width:48px;height:48px;object-fit:contain}.objectCard span{font-weight:1000}.objectCard small{font-size:12px;color:#52677c}.objectCard em,.objectCard b{font-size:11px;border-radius:999px;padding:3px 7px;background:#eaf4ff;border:1px solid #9fc7ef;font-style:normal}.object1{left:4%;top:8%}.object2{left:26%;top:18%}.object3{left:53%;top:9%}.object4{left:72%;top:27%}.object5{left:10%;top:48%}.object6{left:36%;top:55%}.object7{left:61%;top:49%}.object8{left:78%;top:5%}.motionFlying{animation:floaty 3.2s ease-in-out infinite}.motionFloating{animation:floaty 4.2s ease-in-out infinite}.motionBouncy{animation:bouncy 2.1s ease-in-out infinite}.motionIdle{animation:idle 5s ease-in-out infinite}.speechBubble{position:absolute;left:16px;right:16px;bottom:12px;z-index:3;background:rgba(255,255,255,.94);border:2px solid #203142;border-radius:8px;padding:10px;box-shadow:4px 4px 0 rgba(32,49,66,.2);max-height:88px;overflow:auto}.speechBubble strong{display:block;margin-bottom:3px}.ideaForm{display:flex;gap:8px;margin-top:8px;position:relative;z-index:6}.ideaForm input{flex:1;border:2px solid #203142;border-radius:8px;padding:12px;font-size:16px;background:#fff}.spellInput button{background:#ff8c6a;color:white}.starterIdeas{display:flex;flex-wrap:wrap;gap:8px;margin-top:8px}.sparkSpellbook button{background:#eefbf7}.npcRequests p,.timelineCard,.summaryCard{border:2px solid #dce7f1;border-radius:8px;padding:10px;background:#fbfdff}.inventoryShelf{display:flex;flex-wrap:wrap;gap:8px}.inventoryToken{display:inline-grid!important;grid-template-columns:28px auto;align-items:center;gap:6px}.inventoryToken img{width:28px;height:28px}.floatingCreatorTray select{width:100%;border:2px solid #203142;border-radius:8px;padding:10px;background:#fff;margin-bottom:8px}.floatingCreatorTray>button{display:block;width:100%;margin:8px 0;background:#fff7e8}.parentBook,.testPanel{margin:14px}.testPanel article{margin:8px 0;padding:12px;border-radius:8px;border:2px solid #203142}.pass{background:#e9f9ef}.wait{background:#fff6e5}.causalLab{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0 12px}.causalLab span{border:1px solid #bad8ee;background:#eef8ff;border-radius:8px;padding:7px 8px;font-weight:800;color:#24435d}@keyframes floaty{0%,100%{transform:translateY(0) rotate(-1deg)}50%{transform:translateY(-13px) rotate(2deg)}}@keyframes bouncy{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-8px) scale(1.04)}}@keyframes idle{0%,100%{transform:rotate(0deg)}50%{transform:rotate(1deg)}}@media(max-width:1000px){.playGrid,.immersiveGameShell{grid-template-columns:1fr}.stagePanel{margin-left:0;margin-right:0}.floatingQuestRail,.floatingCreatorTray{position:relative;left:auto;right:auto;top:auto;width:100%;max-height:none}.stageScene{height:500px}.topBar{align-items:flex-start;flex-direction:column}.worldHeader{grid-template-columns:82px 1fr}.shardBadge{grid-column:1/-1}}@media(max-width:560px){.playGrid{padding:8px}.stageScene{height:430px}.objectCard{min-width:96px;min-height:102px}.ideaForm{flex-direction:column}}
"""
...
def _object_svg(color: str, kind: str) -> str:
    shapes = {
        ...
        "vehicle": '<path d="M30 94h100l-12-30H54z" fill="{color}" stroke="#203142" stroke-width="7"/><circle cx="56" cy="116" r="14" fill="#fff" stroke="#203142" stroke-width="7"/><circle cx="108" cy="116" r="14" fill="#fff" stroke="#203142" stroke-width="7"/><path d="M62 64h34l8 30H48z" fill="#fff" stroke="#203142" stroke-width="6"/>',
        "music": '<path d="M58 111V42l60-12v68" fill="none" stroke="#203142" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/><circle cx="44" cy="118" r="18" fill="{color}" stroke="#203142" stroke-width="7"/><circle cx="104" cy="104" r="18" fill="{color}" stroke="#203142" stroke-width="7"/>',
