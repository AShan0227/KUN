    ...
    *,
    ...
    return [
        ...
        DeliveryCapability(
            ...
            label="KUN 编译层",
            status="partial",
            summary="已有轻量输入编译器、CLI/AssetStore 写入入口、批量资料 manifest、聊天附件热路径和 Hermes 标准材料包；可处理文本/Markdown/HTML/JSON/CSV、本地 PDF 摘要，以及可选白名单 URL 文本抓取，但还没接 Office/OCR/音视频重后端。",
            done=[
        ...
        DeliveryCapability(
            ...
            missing=[
                ...
                "并发车道已在 MultiTaskScheduler 内生效，但还没把所有 Orchestrator / worker / cron 队列统一切到这一套调度入口",
            ],
            next_steps=[
        ...
        DeliveryCapability(
            ...
            missing=[
                ...
                "Qi idle replay 目前已有 heuristic_local、可配置本地模型评估口、显式 opt-in 强模型复审口、显式 opt-in KUN-Lab 历史任务回放口和草稿审核状态机；但仓库不内置具体模型权重，也还没接 shadow/canary 推广链路",
                "StrategyPack 草稿目前只做 review-only，不会自动 promotion；已经能判断是否可交给人审核，但还没接人工批准 UI",
                "external_emergent_scan 目前只消费显式线索；还没接真实 GitHub/arXiv/竞品 changelog 抓取器，也还没接强模型复审或 canary 推广链路",
