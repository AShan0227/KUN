    ...
    return [
        ...
        DeliveryCapability(
            ...
            missing=[
                ...
                "Qi idle replay 目前已有 heuristic_local、可配置本地模型评估口、显式 opt-in 强模型复审口和显式 opt-in KUN-Lab 历史任务回放口；但仓库不内置具体模型权重，也还没接 shadow/canary 推广链路",
                "StrategyPack 草稿目前只做 review-only，不会自动 promotion；强模型复审已有 opt-in 后台口，但还没接人工批准 UI",
