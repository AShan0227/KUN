    ...
    return [
        ...
        DeliveryCapability(
            ...
            summary="已有轻量输入编译器、CLI/AssetStore 写入入口、批量资料 manifest、聊天附件热路径、Hermes 标准材料包，以及 skill/task/protocol 内部资产编译器；可处理文本/Markdown/HTML/JSON/CSV、本地 PDF 摘要、KUN 内部对象和可选白名单 URL 文本抓取，但还没接 Office/OCR/音视频重后端。",
                ...
                "InputTranslator 的 anchor-expand 第三轮现在支持注入真实 deep_analyzer；未配置时明确 not_configured，失败时明确 unavailable，不再把占位分析说成已理解",
        ...
        DeliveryCapability(
            ...
            summary="已有任务状态、事件、恢复信号的基础，但还不能独立长期运营一个产品。",
                ...
                "黑板提供 state-ledger/credit-summary，把多任务信用归因汇总成 top 资源类型和具体资源 key，供傩/启判断哪些模型、skill、context 反复真有用",
        ...
        DeliveryCapability(
            ...
            summary="健康、成本、权限、风险是默认入口；高级诊断和能力画像已折叠。",
                ...
                "NUO governance_recommendations 已有显式 apply 入口：目前只允许低风险 context maintenance dry-run/apply，高风险或需要人审的建议会返回结构化 blocked、action ticket 和 nuo_diagnosis DecisionTicket",
                "NUO governance apply/block 决策会尽力写入 StateLedger 和 EventRow，方便后续审计“傩为什么允许或阻止一次治理动作”",
                "诊断 confirm API 会区分“用户已确认”和“真实执行器已运行”：没有 handler 时只记录确认并明确说明不会假装修复完成",
            ...
            evidence_refs=[
                ...
                "tests/unit/test_fix_handlers.py",
            ],
            missing=[
        ...
        DeliveryCapability(
            ...
            summary="已有结果、过程、元决策写回的第一版，复用闭环还要继续打通。",
                ...
                "StateLedger 可把近期多任务故事里的 credit assignment 聚合成跨任务摘要，让 MoE 路由先看哪些资源长期有效，而不是每次从零判断",
                ...
                "MemoryInvocationPolicy 已把广告/内容/转化类任务单独归为 marketing 路径，优先激活方法论、行为模式、历史结果和创意/转化标签，不再和普通问答一锅炖",
                ...
                "没有本地命令时，可显式开启 KUN_QI_CHEAP_REPLAY_ENABLED=1，让启走 KUN cheap/judge 路由做低成本复盘；这类 LLM 成本会计入 Qi 日预算",
                "启 idle replay 可通过 KUN_QI_STRONG_REVIEW_ENABLED=1 显式接入强模型 judge，对高风险 StrategyPack 草稿做 review-only 复审，并把评审记录沉淀到草稿资产",
                "强模型复审同样受 Qi 日预算约束；预算不足时只产出 budget_exhausted 记录，不继续烧后台成本",
                ...
                "启 rollout plan step 会给 ready_for_human_review 草稿生成 shadow→canary→rollout 护栏计划；显式开启 KUN_QI_ROLLOUT_EXPERIMENT_CREATE_ENABLED=1 且草稿带人工批准标记后，可创建 draft 实验记录，但仍不启动生产流量",
            ...
            evidence_refs=[
                ...
                "tests/unit/test_system_governance_audit.py",
            ...
            missing=[
                ...
                "Qi idle replay 目前已有 heuristic_local、可配置本地模型评估口、显式 opt-in 强模型复审口、显式 opt-in KUN-Lab 历史任务回放口、显式 opt-in AI Scientist tree search 证据、草稿审核状态机、shadow/canary 护栏计划、人工批准后的 draft 实验创建口和守望影子观测；但仓库不内置具体模型权重，也还没接真实流量 canary 执行链路",
                "StrategyPack 草稿不会自动 promotion；已经能判断是否可交给人审核、生成推广计划、人工批准后创建 draft 实验，并进入守望 shadow 观测，但还没接人工批准 UI 或真实 canary 流量",
        ...
        DeliveryCapability(
            ...
            status="partial",
            summary="reader / writer / executor / debugger / reviewer 已有基础模块；CodeCapability 现在会安装到 API runtime，并提供只读 review/diff、显式 sandbox run/check、默认 dry-run 的单文件改写工作流，以及代码文件引用的 Orchestrator 主路径只读预检。它还不是完整 autonomous coder，不能声称能自主大范围改仓库或自动晋升 skill。",
            done=[
                ...
                "CodeCapability 的 strategy_search_records 也会写成 Qi review-only 问题信号，让启能继续复盘“哪条编程路线更好”，而不是只埋在 draft skill 里",
                ...
                "Orchestrator 主路径的 proactive tool dispatch 已接 code-review：用户任务里直接引用代码文件时，会先做只读 CodeCapability 预检，并把 review 结果塞给 LLM；不会写文件、不会执行代码",
            ],
            evidence_refs=[
                ...
                "kun/engineering/proactive_tools.py",
                "rules/proactive/triggers.yaml",
                "kun/api/runtime.py",
                ...
                "tests/unit/test_proactive_tools.py",
            ...
            missing=[
                ...
                "Orchestrator coding task 已能候选 code-review / code-propose-change，也能对显式代码文件做只读预检；但还没把自动生成补丁、skill draft 审批晋升串成完整 autonomous coding workflow",
                "还没接长周期 dogfood 样本来校准何时生成临时代码、何时沉淀为 skill",
            ],
            next_steps=[
                "继续把 Orchestrator coding task 的补丁生成保持在 dry-run / 审批 / canary 链路内，不能静默真实写库",
