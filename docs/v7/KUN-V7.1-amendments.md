...
|---|---|---|---|---|
| A1 | §4.3 | EngineeringDiscipline runtime wiring | P0 ⚠️ (downgraded) | ✅ X.R |
| A2 | §11.4 | Ensemble cost multiplier real-LLM evidence | P1 | ✅ X.R |
| A3 | §12.2 | TicketVerifier Protocol (CANARY → PRODUCTION) | **P0** | ✅ X.P |
| A4 | §12.3 | RSI write-side ProcessAudit closure (X.M) | P1 | ✅ X.R |
| A5 | §12.4.2 | Trifecta past line `bug_root_cause_cases` lookup | P1 | ✅ X.R |
...
| A8 | §15.3 (new) | Production-entry diff check + ProductionEntryDiffChecker | P1 | ✅ X.R |
...
| A11 | §16.5 | runtime_features_used trace shape | P1 | ✅ X.R |
| A12 | §16.6 | Angle 8 production-path-traceability + hidden-orphan-audit template | **P0** | ✅ X.P |
| A13 | §16.8 | X.H AST CI + X.N template lint as required automation | P1 | ✅ X.R |
| A14 | §20 | Cockpit reader pattern + discipline_store + ensemble reader | P1 | ✅ X.R |
| A15 | §23.2 | Acceptance: AST CI + template lint pass | P1 | ✅ X.R |
| A16 | Appendix A | X.A → X.O phase entries | P2 | ✅ X.R |
| A17 | Appendix B | New terms (hidden orphan / chain-reach / etc.) | P2 | ✅ X.R |
