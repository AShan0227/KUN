...
更准确地说，NUO 是整个 KUN 系统的管家，不只是 agent 管家。

Agent 本质上是 context、记忆、模型、skill、工具、权限、策略和运行状态的临时组合，所以只诊断 agent 是不够的。NUO 的体检对象必须覆盖：

- agent：能力画像、失败率、成本、擅长/不擅长的任务类型。
- context：相关性、冗余、过期、污染、压缩质量、是否被错误注入。
- memory：结果记忆、过程记忆、元决策记忆的复用率、可信度、过期度和瘦身空间。
- engineering：Orchestrator、State Ledger、Decision Ticket、事件流、预算、验证、恢复、调度有没有真实闭环。
- agent 协作：多 agent 是否真的提升效果，是否重复劳动，是否互相冲突，交接信息是否足够。
- 外部交互：World Gateway handler 是否可靠，审批是否清楚，真实外发是否可审计、可补偿。
- 策略系统：Strategy Pack、模型路由、评估指标、credit assignment 是否真的影响下次决策。
- 安全和权限：跨租户、越权、SSRF、路径穿越、提示注入、数据投毒、外部动作风险。

NUO 不能只展示“现在正常/不正常”。它要定期做深度复盘和系统瘦身：

- 找出长期不用、低贡献、重复、污染的 context 和记忆。
- 给出压缩、合并、降权、遗忘、永久保留的建议。
- 找出“功能存在但没人消费”的伪功能。
- 找出协作链路里的重复执行和冲突。
- 找出外部动作里的高风险 handler、失败 handler、缺少补偿策略的 handler。
- 找出高成本低收益的模型/skill/context 组合。

NUO 的建议不能直接越权执行。标准链路是：
...
对应模块执行修复
  ↓
State Ledger / Event / Memory 记录“修了什么、为什么、效果如何”
```

这能避免 NUO 变成另一个“什么都管、什么都执行”的超级模块。
...
- NUO 后台能产出 agent / context / memory / engineering / collaboration / WorldGateway 体检结果，但默认不打扰用户。
...
- NUO 定期体检：发现工程链路断点、agent 协作冲突、外部交互风险、策略评分失真和伪功能。
...
- NUO 能指出至少一类“系统链路问题”：例如写回没有消费、事件没有订阅、协作重复、外部动作缺少补偿。
