...
| Qi problem queue | 已接 idle replay，能读真实问题信号和任务结果历史；NUO health findings 会写入 Qi problem queue，Qi StrategyPack 草案状态也会回到 NUO governance report；cheap 路由评估和强模型复审都受 Qi 日预算约束；CodeCapability 策略搜索记录会进入 Qi review-only 队列 | 还要接人工批准 UI 和真实 canary 实验 |
