...
| **F134** | `.github/` 下只有 `workflows/ci.yml` 一个文件——无 CODEOWNERS、无 branch-protection-as-code。且 CI 只在 `main` 触发，**真正在动的 `鲲V1.1-dev` 无 CI**；即便 main 配了 required checks 也保护不到开发分支。 | (a) CI 触发加上活跃开发分支；(b) 加 CODEOWNERS；(c) 文档化 required status checks(lint/unit+cov/typecheck/integration)并在仓库设置开启(GitHub 设置项，代码无法核实，需人操作 + 文档留痕)。 |
