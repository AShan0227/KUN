...
| **F109** | `kun/skills/sandbox.py:43-63` `resolve_execution_cwd` 仅校验 cwd 落在 configured roots 内（docstring 自承"This is not a container"），无进程隔离、无 fs/network/syscall 限制；roots 由调用方 cwd 可放大。prompt-injected 代码在边界内可任意读写、联网、起子进程。 | 真隔离：容器 / seccomp / nsjail / 只读 rootfs + 无网络 namespace + 资源 cgroup；执行器接受能力声明白名单。属基础设施级改动。 |
...
## 3. 覆盖 findings
F102, F109, F148, F151（标 needs-design 指向本文件）。
