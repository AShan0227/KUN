...
```bash
uv run kun ops release-check --tag v5.0.0 --require-ready
```
...
# 修复 + 测试
uv run kun ops release-check --tag v5.0.1 --require-ready
git tag -a v5.0.1 -m "KUN v5.0.1 hotfix"
...
## 7. Delivery Honesty / Dogfood / Secrets

V5 发版前必须额外确认：

```bash
uv run kun ops delivery-status --json
uv run kun ops dogfood --tenant <tenant> --include-db-account --include-db-mission --include-db-state-ledger-repair
uv run kun ops preflight --require-recent-backup-drill
uv run python scripts/check_legal_guard.py
```

要求：

- `delivery-status` 里任何 `partial` / `not_ready` 都必须继续对用户诚实标注，不能写成 ready。
- `production_deployment` 如果仍是 `not_ready`，只能发内部测试版，不能宣称生产级完成。
- dogfood 报告要保存到发版记录里，至少覆盖账号、Mission、StateLedger、WorldGateway 低风险动作。
- secret store / `KUN_SECRET_STORE_FILE` / 租户级 WorldGateway 密钥配置不能写进仓库。
- 真实生产 secret 仍然要迁到云 KMS / Secret Manager；本地 JSON 只允许内测和开发环境。
- Legal / IP guard 必须通过，公开仓库不能包含客户数据、密钥、内部 GTM、投资材料或未公开商业方案。
