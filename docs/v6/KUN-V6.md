...
复杂任务必须先形成任务方案。任务方案不是展示文档，而是运行时对象。后续拆解、执行、评估、合并、交付和返工都必须引用当前任务方案版本。
...
```text
1. Intake：接收用户目标
2. Understand：编译目标、上下文、资料和历史记忆
3. Plan：生成任务方案，识别信息缺口
4. Align：补齐信息并获得必要确认
5. Contract：生成执行合同
6. Decide：选择策略、风险等级、资源和执行路径
7. Decompose：拆解为可执行工作项
8. Queue：进入持久队列
9. Execute：由 supervisor 调度工具、worker、外部 agent 或人
10. Observe：记录状态、证据、日志、成本、风险
11. Evaluate：按北极星和任务类型 rubric 生成 GateEvaluation
12. Repair：修复、回滚、改计划、复测或转人工
13. Merge：合并多 worker、证据、测试和评审输出
14. Deliver：编译可验收交付物
15. Accept：用户或 reviewer 验收、返工、partial 或拒绝
16. Learn：评估、信用分配、能力写回
17. Govern：系统健康治理、瘦身、权限和成本管理
```

Plan Alignment 贯穿 1 到 17。只要目标、验收、风险、成本、证据或执行路线偏离当前任务方案，就必须触发计划变更协议。

### 3.1 七个必须闭合的回路

| 回路 | 第一性问题 | 闭环路径 |
| --- | --- | --- |
| 目标回路 | 用户到底要什么结果？ | 目标 -> 任务方案 -> 信息补齐 -> 确认 -> 执行合同 |
...
`CapabilityProfile` 不属于最小执行闭环，但属于完整 KUN 产品闭环；当 KUN 开始自我学习和能力晋级时必须启用。
