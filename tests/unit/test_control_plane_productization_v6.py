...
def _promote_productization_capability(
    ...
    *,
    ...
    promotion = build_capability_promotion(
        candidate,
        [_capability_evaluation(stage) for stage in ["replay", "holdout", "shadow", "canary", "production"]],
...
def test_productization_dogfood_execution_runs_queue_and_ab_regression(tmp_path) -> None:
    ...
    assert "real_long_task_dogfood" in recovered.artifacts[
        execution.execution_report_artifact_ref
    ].supports
    assert "cross_restart_resume" in recovered.artifacts[
        execution.recovery_bundle_artifact_ref
    ].supports
    ...
    assert "qi_capability_evolution" in closed.artifacts[
        acceptance.learning_artifact_ref
    ].supports
...
def test_external_behavior_distillation_creates_kun_native_candidates() -> None:
    ...
    assert "openclaw:explicit-approval-interaction-with-resumable-tickets:collaboration_tickets" in refs
    assert "hermes:activity-based-long-run-timeout-instead-of-wall-clock-kill:persistence_recovery" in refs
    ...
    assert all("kun-control-plane:" in candidate.proposed_change_refs[0] for candidate in candidates)
    assert all(candidate.target_task_types == ["self_improvement", "ops_tooling"] for candidate in candidates)
...
def test_productization_code_boundary_audit_accepts_clean_submission(tmp_path) -> None:
    ...
    assert "artifacts/control_plane/kun-v6-productization-dogfood.json" in report.artifact_state_paths
