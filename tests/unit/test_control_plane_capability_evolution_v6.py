...
def _evaluation(stage: str, **overrides: object) -> CapabilityEvaluation:
    ...
    if stage in {"canary", "production"}:
        ...
        payload["rollback_plan"] = [
            "disable capability profile",
            "restore previous runtime routing",
        ]
    payload.update(overrides)
...
def test_runtime_default_capabilities_only_load_production_profiles() -> None:
    ...
    assert [
        profile.capability_id for profile in recovered_runtime.list_default_runtime_capabilities()
