...
def test_memory_store_round_trips_all_v6_records_and_mission_scoped_lists() -> None:
    ...
    assert [
        contract.contract_id for contract in store.list_execution_contracts(mission_id="msn-a")
    ] == ["contract-msn-a"]
    assert [
        context.working_context_id for context in store.list_working_contexts(mission_id="msn-a")
    ] == ["ctx-msn-a"]
